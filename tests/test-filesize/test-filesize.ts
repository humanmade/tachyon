...
const table = new Table({
	head: ['Image', 'Original Size', 'Tachyon Size', '100px', '300px', '700px', '700px webp', '700px avif'],
	style: {
		compact: true,
	},
	colWidths: [15, 15, 20, 10, 10, 10, 17, 17],
});
...
test('Test file sizes', async () => {
	...
	for (const key in fixtures) {
		...
		if (fixtures[key] !== oldFixtures[key]) {
			...
			console.log(
				`${key} is different than image in fixtures by (${filesize(
					oldFixtures[key] - fixtures[key]
